...
class EditTogetherAge extends StatelessWidget {
  const EditTogetherAge({super.key});
  ...
  void _onChangeAge(
    ...
    if (age == null) return;
    context.read<EditTogetherBloc>().add(EditTogetherChangeAge(
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<EditTogetherBloc, EditTogetherState>(
      buildWhen: (previous, current) =>
          previous.together.age != current.together.age,
      builder: (context, state) => AppDropdown(
        hint: '적정 연령을 골라주세요',
        width: double.infinity,
        value: state.together.age?.text ?? EChildAge.allAge,
        values: [
          EChildAge.allAge,
